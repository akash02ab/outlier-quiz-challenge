...
const Score = () => {
  ...
  const lowestScore = Math.round(answeredCorrectly / totalQuestions * 100) + '%'
  const remainingQuestion = totalQuestions - answered
  const maxScore = Math.round((answeredCorrectly + remainingQuestion) / totalQuestions * 100) + '%'

  return (
    <div>
      <div className='d-flex align-items-center justify-content-between mb-2'>
        <h6>Score: {currentScore}</h6>
        <h6>Max Score: {maxScore}</h6>
      </div>
